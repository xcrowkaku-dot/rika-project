...
function startBot() {
  ...
  login(credentials, config.loginOptions, async (err, api) => {
    ...
    api.onReLoginFailure = async (e) => {
      ...
      logger.info("Bot", "Restarting process in 60s...");
      ...
      if (mqttErr) {
        ...
        if (_mqttErrorCount >= MQTT_CONFIG.ERROR_THRESHOLD) {
          ...
          if (_mqttRestartCount > MQTT_CONFIG.MAX_RESTART_ATTEMPTS) {
            logger.fatal("MQTT", "Exceeded " + MQTT_CONFIG.MAX_RESTART_ATTEMPTS + " restart attempts — shutting down.");
            setBotStatus("offline — too many MQTT failures");
            cookieRefresher.stop();
            humanSimulator.stop();
            process.exit(1);
          }
